...
//           API.get("admin/reports/restaurants"),
//           API.get("admin/reports/customers"),
//           API.get(`admin/reports/top-categories?month=${monthMap[selectedMonth]}`),
//         ]);
...
//   if (error) return <ErrorState error={error} onRetry={() => fetchReports(month)} />;
...
//             </div>
//             <span className="text-white/90 font-semibold">Analytics Dashboard</span>
//           </div>
...
// /* Stats Card */
// const StatsCard = ({ icon, value, label, subtitle, gradient, trend, delay }) => (
//   <motion.div
...
//   const rankStyle = rankIcons[rank] || { icon: `#${rank}`, color: "from-slate-400 to-slate-500" };
...
//     : 0;
//   const avgDailyOrders = totalDays > 0
//     ? salesTrends.reduce((sum, t) => sum + t.totalOrders, 0) / totalDays
//     : 0;
//   const peakDay = salesTrends.reduce((max, t) =>
//     t.totalRevenue > (max.totalRevenue || 0) ? t : max, {});
...
//                 isRevenue ? "from-purple-500 to-indigo-600" : "from-pink-500 to-rose-600"
//               } flex items-center justify-center shadow-lg`}
//             >
//               {isRevenue ? <FaChartBar className="text-white" /> : <FaPizzaSlice className="text-white" />}
//             </div>
//             <h3 className="text-xl font-bold text-gray-900 dark:text-white">
