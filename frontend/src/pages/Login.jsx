...
export default function Login() {
  ...
  // refs to the two card wrappers we'll equalize heights for visual parity
  ...
  // Sync heights of the two card wrappers so both cards visually match.
  // This enforces equality only on md+ (>= 768px). On smaller viewports we clear inline heights.
  useLayoutEffect(() => {
    const leftEl = leftWrapperRef.current;
    const rightEl = rightWrapperRef.current;
    if (!leftEl || !rightEl) return;

    function syncHeights() {
      if (window.innerWidth < 768) {
        // mobile / stacked layout: let them size naturally
      ...
      // set identical pixel height on both wrappers (prevents internal scrollbars/clipping)
      leftEl.style.height = `${max}px`;
      rightEl.style.height = `${max}px`;
    }
    ...
    try {
      ...
      // ResizeObserver might not be available — fallback handled by resize event below
    }

    window.addEventListener("resize", syncHeights);

    // ensure images loading later trigger sync
    const imgs = [
      ...leftEl.querySelectorAll("img"),
      ...rightEl.querySelectorAll("img"),
    ];
  ...
  return (
    <div className="relative min-h-screen overflow-hidden bg-gradient-to-br from-orange-50 via-pink-50 to-purple-50 dark:from-slate-950 dark:via-slate-900 dark:to-slate-950">
      ...
      {/* Grid Container */}
      <div className="relative z-10 pt-20 min-h-screen grid grid-cols-1 md:grid-cols-2 items-center justify-center gap-6 px-6 lg:px-12 transition-colors duration-500">
        {/* Dark Mode Toggle */}
        <motion.button
        ...
        >
          ...
          <div
            ref={leftWrapperRef}
            className="w-full max-w-lg h-auto flex items-center justify-center"
            /* height is managed by useLayoutEffect on md+ */
          >
            <div className="relative w-full h-full rounded-3xl shadow-2xl border border-orange-200 dark:border-white/10 bg-white/80 dark:bg-slate-900/80 p-10 overflow-hidden">
              <div className="absolute inset-0 rounded-3xl pointer-events-none">
                <div className="w-full h-full rounded-3xl bg-gradient-to-br from-orange-500/30 to-pink-500/30 blur-3xl animate-pulse"></div>
              </div>
              ...
              <div className="relative z-10 w-full h-full flex flex-col justify-between">
                <div>
                  ...
                  <div className="text-center space-y-4 mt-6">
                    ...
                    <p className="text-lg text-gray-600 dark:text-gray-300">
                      Fast. Delicious. At your doorstep. Sign in and satisfy
                      your cravings! 🍔✨
                    </p>
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-4 mt-6">
                    {[
                      ...
                      { icon: <FaHeart />, text: "Best Quality" },
                      { icon: <FaRocket />, text: "Quick Delivery" },
                      { icon: <FaBolt />, text: "Top Restaurants" },
                    ].map((feature, i) => (
                      <motion.div
                        key={i}
                ...
                <div>
                  <img
                    src="/QuickBite2.jpg"
                    alt="Delicious food"
                    className="rounded-2xl shadow-xl border border-orange-200 dark:border-white/10 hover:scale-105 transition-transform duration-500 mt-6 w-full"
                    onError={(e) => (e.target.style.display = "none")}
                  />
        ...
        >
          {/* wrapper: use same max width as Register (max-w-lg) so widths match exactly */}
          <div
            ref={rightWrapperRef}
            className="w-full max-w-lg h-auto flex items-center justify-center"
            /* height is managed by useLayoutEffect on md+ */
          >
            <div className="w-full h-full rounded-3xl shadow-2xl border border-orange-200 dark:border-white/10 bg-white/90 dark:bg-slate-900/90 p-10 overflow-hidden flex flex-col">
              {/* card content: no internal scrolling */}
              <div className="w-full h-full flex flex-col justify-between">
                ...
                <div className="mt-6 text-center">
                  ...
                  <p className="text-xs text-gray-500 dark:text-gray-400 mt-3 px-4">
